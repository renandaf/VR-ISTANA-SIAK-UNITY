...
public class InspectStart : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        _originalObjectRotation = inspectObject.transform.rotation.eulerAngles;
        ...
        _outline = inspectObject.GetComponent<Outline>();
    ...
    private void Update()
    {
        if (!_isVisible)
        {
            ...
            targetDir.y = 0;
            Vector3 newDir = Vector3.RotateTowards(inspectObject.transform.forward, targetDir, 1.3f * Time.deltaTime, 0.0F);
    ...
    private void MoveToward(Vector3 targetPosition)
    {
        ...
        informationUI.transform.position = Vector3.SmoothDamp(informationUI.transform.position, targetPosition, ref _velocity2, 1f);
        if (inspectObject.transform.localScale != targetScale && targetScale != Vector3.zero)
        {
            inspectObject.transform.localScale = Vector3.SmoothDamp(inspectObject.transform.localScale, targetScale, ref _velocity3, 1f);
        }
    }
    ...
    public void EnterView()
    {
        ...
        LeanTween.value(_inspectArea.gameObject, 0, 0.98f, 2f).setOnUpdate((float val) =>
        {
            Color c = _mat.color;
            c.a = val;
            _mat.color = c;
    ...
    {
        LeanTween.value(_inspectArea.gameObject, 0.98f, 0, 2).setOnUpdate((float val) =>
        {
            Color c = _mat.color;
            c.a = val;
            _mat.color = c;
        });
        LeanTween.move(inspectObject.transform.gameObject, _originalPosition, 2);
        LeanTween.scale(inspectObject.gameObject, _originalSize, 2);
        LeanTween.rotate(inspectObject.transform.gameObject, _originalObjectRotation, 2);
        ToggleInspect();      
